...
public class MovingSphere : MonoBehaviour, IControllableBody {
	...
	[SerializeField, Min(0f)] float sprintMax = 2;
	...
	public void BodyUpdate(BodyInput bodyInput) {
		playerInput = bodyInput.moveInput;
		if (!Swimming) {
			playerInput.y = 0;
		}
		...
		if (bodyInput.sprint) {
			sprint = Mathf.MoveTowards(sprint, sprintMax, Time.deltaTime * 5);
		} else {
			sprint = Mathf.MoveTowards(sprint, 1, Time.deltaTime * 5);
		}

		if (bodyInput.moveAxis) {
			rightAxis = ProjectDirectionOnPlane(bodyInput.moveAxis.right, upAxis);
			forwardAxis =
				ProjectDirectionOnPlane(bodyInput.moveAxis.forward, upAxis);
		}
		else {
		...
		if (Swimming) {
			...
			desiredJump |= bodyInput.jump;
			desiresClimbing = bodyInput.climb;
		}
	...
	void UpdateSounds() {
		...
		var curVelocity = body.velocity * playerInput.magnitude;
		...
		_audioPlayer.SetPitch((curVelocity.magnitude/10 + Mathf.PerlinNoise(Time.time*5,0)/3f)/2f);
		//print(curVelocity.magnitude/10f);
	}
	...
	void AdjustVelocity () {
		...
		Vector3 adjustment;
		adjustment.x =
			playerInput.x * speed *sprint - Vector3.Dot(relativeVelocity, xAxis);
		adjustment.z =
			playerInput.z * speed *sprint - Vector3.Dot(relativeVelocity, zAxis);
		adjustment.y = Swimming ?
			playerInput.y * speed *sprint - Vector3.Dot(relativeVelocity, upAxis) : 0f;
