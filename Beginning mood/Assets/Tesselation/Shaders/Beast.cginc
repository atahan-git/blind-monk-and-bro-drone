...
UnityTessellationFactors hsconst_surf (InputPatch<InternalTessInterp_appdata,3> v) 
{
	...
	float tessEdgeLength = _Beast_TessellationEdgeLength;
			...
			tessFactor *= _Beast_TessellationShadowPassLOD;
				...
				tessEdgeLength = clamp(_Beast_TessellationEdgeLength * _Beast_TessellationShadowPassLOD, 2, 64);
			#endif
			...
			#if defined(_BEAST_TESSELLATION_TYPE_EDGE_LENGTH) || defined(_BEAST_TESSELLATION_TYPE_PHONG)
				tessEdgeLength = clamp(_Beast_TessellationEdgeLength * _Beast_TessellationDepthPassLOD, 2, 64);
			#endif
	...
	tessFactor = clamp(tessFactor, 1, 64);
		...
		#else
			tf = UnityEdgeLengthBasedTess(vi[0].positionOS, vi[1].positionOS, vi[2].positionOS, tessEdgeLength);
		#endif
...
inline float UnpackDepth(float2 uv, float3 vertexPos, float3 normal)
{
		...
		return (cx[_Beast_TessellationDisplaceMapChannel] * blend.x + cy[_Beast_TessellationDisplaceMapChannel] * blend.y + cz[_Beast_TessellationDisplaceMapChannel] * blend.z) * _Beast_TessellationDisplaceStrength;
		 
	#else

		float4 c = SAMPLE_TEXTURE2D_LOD(_Beast_TessellationDisplaceMap, sampler_Beast_TessellationDisplaceMap, uv, 0);
...
inline void Displace(inout float4 vertex, inout float3 normal, inout float4 tangent, float2 texcoord, float3 smoothNormal)
{
	float3 displaceVector = _Beast_TessellationUseSmoothNormals > 0.5 ? smoothNormal : normal;
		...
		v0 += displaceVector * UnpackDepth(uv0, vertex.xyz * _Beast_TessellationTriplanarUVScale, normal);
		...
		v1 += displaceVector * UnpackDepth(uv1, vertex.xyz * _Beast_TessellationTriplanarUVScale +  float3(0, _Beast_TessellationNormalCoef * (vertex.y > 1 ? 0.1 : -0.1), 0), normal);

		float2 uv2 = uv0 + float2(0, offset.y);
		v2 += displaceVector * UnpackDepth(uv2, vertex.xyz * _Beast_TessellationTriplanarUVScale + float3(0, 0, _Beast_TessellationNormalCoef * (vertex.z > 1 ? 0.1 : -0.1)), normal);
		...
		vertex.xyz += displaceVector * UnpackDepth(uv, vertex.xyz * _Beast_TessellationTriplanarUVScale, normal);
